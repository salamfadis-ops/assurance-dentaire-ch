...
export function getDocumentMaxSizeBytes() {
  ...
  const megabytes = Number.isFinite(configured) && configured > 0 ? configured : DEFAULT_DOCUMENT_MAX_SIZE_MB;
...
export function validatePdfSelection(files: Array<{ type: string; size: number }>, remainingSlots: number) {
