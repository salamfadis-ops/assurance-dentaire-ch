...
export function ProtectionVisual() {
  ...
  return (
      <div className="relative mx-auto aspect-[0.92] w-full max-w-[34rem] lg:max-w-none" role="img" aria-label="Aperçu du Score Protection Dentaire">
        <div className="absolute inset-[8%] rounded-full bg-[#d8ffef]/15 blur-3xl" />
        <div className="hero-dashboard absolute inset-x-[4%] bottom-[2%] top-[4%] overflow-hidden rounded-[2rem] border border-white/15 bg-[#0d2b27]/90 p-5 shadow-[0_40px_100px_rgba(2,16,14,0.5)] backdrop-blur-xl sm:rounded-[2.5rem] sm:p-7">
            ...
            <div className="relative h-56 w-56 sm:h-64 sm:w-64">
              <svg viewBox="0 0 180 180" className="h-full w-full -rotate-90" aria-hidden="true">
                ...
                <circle
                  ...
                  strokeDashoffset={circumference * 0.21}
                  className="hero-score-ring"
                />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                <strong className="hero-score-value font-display text-6xl font-semibold tracking-[-0.07em] text-white sm:text-7xl">79</strong>
          ...
          <div className="absolute bottom-5 left-5 right-5 grid grid-cols-3 gap-2 sm:bottom-7 sm:left-7 sm:right-7">
            {[["Budget", "Maîtrisé"], ["Contrat", "À clarifier"], ["Prévention", "Solide"]].map(([label, value], index) => (
              <div key={label} style={{ animationDelay: `${0.75 + index * 0.1}s` }} className="hero-metric rounded-xl border border-white/10 bg-white/[0.055] p-2.5 sm:p-3">
        ...
        <div className="hero-float hero-float-right absolute right-0 top-[22%] hidden items-center gap-2 rounded-2xl border border-white/15 bg-white/95 px-3.5 py-3 text-xs font-bold text-[#12332d] shadow-2xl sm:flex">
        ...
        <div className="hero-float hero-float-left absolute -left-2 bottom-[24%] hidden items-center gap-2 rounded-2xl border border-white/15 bg-[#f7a57d] px-3.5 py-3 text-xs font-bold text-[#32150c] shadow-2xl sm:flex">
...
export function JourneyTimeline() {
  ...
  return (
      <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:gap-14">
        ...
        <div key={item.id} className="assessment-step-enter relative min-h-[23rem] overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.055] p-7 sm:p-10">
