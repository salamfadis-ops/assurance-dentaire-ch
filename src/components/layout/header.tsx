  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/[0.08] bg-[#071c19]/95 text-white backdrop-blur-xl">
      <Container className="flex h-20 items-center justify-between gap-6">
        ...
        <span className="hidden border-l border-white/10 pl-5 xl:inline-flex"><VydaMark inverse compact /></span>
      ...
      {open && (
        <nav id="navigation-mobile" className="border-t border-white/10 bg-[#071c19] px-5 py-5 sm:hidden" aria-label="Navigation mobile">
          <div className="grid gap-1">
            ...
            <Link href="/bilan" onClick={() => setOpen(false)} className="mt-3 flex items-center justify-center gap-2 rounded-full bg-[#b9f1dd] px-5 py-3 font-bold text-[#08241f]">Faire mon bilan gratuit <ArrowRightIcon className="h-4 w-4" /></Link>
          </div>
        </nav>
      )}
    </header>
