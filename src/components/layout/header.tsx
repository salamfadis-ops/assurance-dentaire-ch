      ...
      {open && (
        <nav id="navigation-mobile" className="border-t border-white/10 bg-[#071c19] px-5 py-5 sm:hidden" aria-label="Navigation mobile">
          <div className="grid gap-1">
            ...
            <Link href="/bilan" onClick={() => setOpen(false)} className="mt-3 flex items-center justify-center gap-2 rounded-full bg-[#b9f1dd] px-5 py-3 font-bold text-[#08241f]">Être rappelé gratuitement <ArrowRightIcon className="h-4 w-4" /></Link>
          </div>
        </nav>
      )}
    </header>
